...
(function bindViewportSizing() {
  ...
  function apply() {
    ...
    // 壁纸层不用跟着键盘收缩高度（它本来就该常驻铺满整个真实屏幕，跟视口是否
    // 被键盘遮住无关），但漏了一件事：iOS 键盘弹起时，系统会自动把画面往上
    // 卷动一段（露出正在输入的框），这段卷动会让 visualViewport.offsetTop
    // 变成正值——那一段之前只补给了 shell/splash 的 top，壁纸层的 top 一直
    // 钉死在"-6%"没跟着挪，画面卷上去之后壁纸的可见范围跟真实屏幕对不上了，
    // 输入框那一截自然就看不到壁纸、只剩 body 自己的底色。壁纸的 top 也加上
    // 同一个 pan 偏移，跟 shell 用同一套基准，高度还是照旧不用跟着键盘缩。
    if (wallpaper) {
